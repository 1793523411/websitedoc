...
- 属于一种元编程，对目标对象进行拦截，外界对该对象的访问都必须经过这层拦截，提供了一种机制可以对外界访问进行过滤和改写，第一个参数表示拦截对象，第二个参数表示拦截行为，`var proxy = new Proxy(target,handler)`,如果 handler 是一个空对象，没有任何拦截效果，访问 handler 就等同于 target,同一个拦截器可以拦截多个操作，proxy 可以作为其他对象的原型对象，这样当读取原型链上的属性时会进行拦截操作
  ...
  has(target,propKey),
  deleteProperty(target,propKsy),
  ownKeys(target),
  getOwnPropertyDescriptor(target,propKey),
  defineProperty(target,propKeys,propDesc),
...
- es6 在语言规格层面实现了模块功能，并且实现的相当简洁，完全可以取代现有的 CommobJS 和 AMD 规范，成为浏览器和服务器通用的模块解决方案，es6 模块的设计思想是尽量静态化，是的编译时就能确定模块的依赖关系，以及输入输出的变量，commitJS 和 AMD 模块都只能在运行时确定这些东西，比如 CommonJS 模块就是对象，输入是必须查找对象属性
- 由于 es6 模块化是编译时加载使得静态分析成为可能，有了他就能进一步拓展 javascript 的语法比如引入宏和类型检查这些只能靠静态分析实现的功能
- es6 模块自动采用严格模式，不管有没有在模块头部加上`use strict`
- 模块功能主要有两个命令构成:export 和 import,export 命令用于规定模块的对外接口，import 命令用于输入其他模块提供的功能，使用 export 是接口名必须与模块内部变量建立一一对应关系，export 语句输出的接口与其对应的值是动态绑定关系，即可以通过接口渠道米块内部实时的值
- import 具有提升效果，会提升到整个模块的头部并首先执行，import 语句会执行所加载的模块，所以可以直接在 import 后面加上模块，多次导入只会执行一次，最好不要吧 import 和 require 写一个模块里，尽管有 babel，模块整体加载所在的对象应该是可以静态解析的，所以不允许运行时改变
- 使用 default 导出，不需要加大括号，import 导入任意指定名称，也不需要加大括号，default 在一个模块中只能使用一次
- 如果一个模块之中先输入后输出同一个模块，import 语句可以和 export 语句写在一起
- 模块之间可以继承
- 跨模块常量：当常量非常多时可以在一个文件夹的一个文件中合并，然后单独导出这一个文件
- 由于 import 无法在运行时加载模块，所以在取代 require 是就成了一个障碍，import 无法取代 require 的动态加载功能，所以有一个提案引入 import()函数来完成动态加载，import 能接受什么参数，import()就能接受什么参数，区别主要就是后者为动态加载，该函数可以用在任何地方，所加载的模块没有静态连接关系，这也与 import 不同，import()类似于 node 的 require(),区别是前者是异步加载，后者是同步加载，使用场景：按需加载，条件加载，动态的模块路径
- 注意点：import()加载模块成功后，这个模块会作为一个对象当做 then 方法的参数，因此可以用对象的解构赋值语法获取输出接口，如果有 default 也可以使用具名输入的形式，如果想同时加载多个模块可以作为一个数组放在 Promise.all 里面，import()也可用在 async 函数之中

## Module 的加载实现

- 浏览器加载 script，默认同步加载，有两种异步加载方法，defer 或 async，前者页面渲染完加载，后者下载完就执行
- 加载 es6 模块时要声明 type=“moudle”属性，并且此模块默认异步加载，不会造成浏览器堵塞，等于打开了 defer 属性，当然后也可以手动打开 async 异步加载方式，es6 模块也可以在内嵌在网页中，与外部加载脚本完全一致，利用顶层 this 等于 undefined 可以用来测试当前代码是否在 es6 模块之中
- es6 与 commonjs 的差异，前者的模块输出是值得复制，后者输出是指的引用，前者是运行时加载，后者是编译时输出接口，前者加载的是一个对象，该对象只有在脚本运行结束时才会生成，后者不是对象，它的对外接口只是一种定义，在代码解析阶段就会生成
- 值的复制：模块内部的变化就影响不到这个值，值的引用：es6 输入模块变量相当于 unix 中的符号连接，多个模块加载该模块加载的是同一个实例
- node 加载，由于他与 node 自带的 Commonjs 不兼容，所以两者要分开，各自采用各自的加载方案，在静态解析阶段，只要有一行 import 或 export，node 就会认为这是 es6 模块，否则就是 commonjs 模块，如果不输出任何接口，但希望被认为是 es6 模块，可以在脚本上加入`export{}`，这不是输出一个空对象，而是不输出任何接口的 es6 的标准写法
- es6 中顶层 this 指向 undefined，commonjs 模块的顶层 this 指向当前模块，两者查找文件的策略也略有所不同
- **在 node 中使用 import 命令加载 CommonJS 模块，node 会自动将 module.exports 属性当做模块的默认输出环境**，即等于 export default,如果采用整体输入的写法，default 会取代 module.exports 作为输入的接口，通过 bar。defaut 这样的写法才能获取 module.exports，因为 bar 本身不是一个对象，不能当做函数调用，只能通过 bar.default 调用,CommonJS 模块的输出缓存机制在 es6 加载方式下依然有效
- import 加载 Commonjs 模块必须整体输入，不允许只引入某个 Commonjs 模块的某一部分，因为 Commonjs 格式的模块在运行时才能确定具体的接口，而 import 要求编译时就确定这个接口
- **采用 require 加载 es6 模块时，es6 模块的所有输入接口都会成为输入对象的属性**
- 循环加载：a 脚本的执行依赖 b 脚本，而 b 脚本的执行依赖 a 脚本，commonjs 模块遇到循环加载是返回的是当前已经执行的部分的值，而不是代码全部执行后的值，两者可能会有差异，所以输入变量时必须非常小心，所以引入变量时应该全部引入，而不要部分引入，不然会发生值被改写，es6 加载的变量都是动态引入其所在模块的，只要存在引用，代码就能执行
- 除了 babel，es6-module-transpiler 命令可将 es6 模块转为 commonjs 模块或 AMD 模块，SystemJS 是一个垫片库，可以在浏览器内加载 es6 模块和 commonjs 模块和 AMD 模块，将其转为 es5 模块，他在后台调用的是 Google 的 Traceur 转码器，其中 System.import 方法用来异步加载具体的模块，返回的是一个 Promise 对象，可以用 then 指定回调函数

## 编程风格

- let 取代 var
- 全局常量，优先使用 const，在全局环境中，不应该设置变量，只应该设置常量，原因：提醒阅读程序的人，该值是不应该改变的，复合函数式编程思想，运算不改变值而是新建值，这样也有利于将来的分布式，编译器会对 const 进行优化，多用 const 有利于提高程序的运行效率，也就是说 let 和 const 的本质区别是编译器内部的不同处理，长远来看 javascript 可能会有多线程的实现，这时 let 表示的变量只应该出现在单线程运行的代码中，不能是多线程共享的，这样有利于保证线程安全
- 静态字符串一律使用单引号或反引号，不使用双引号，动态字符串使用反引号
- 使用数组成员对变量赋值，优先使用解构赋值，函数参数如果是对象的成员，优先使用解构赋值，函数返回多个值，优先使用对象的解构赋值，而不是数组的解构赋值，这便于以后添加返回值以及更改返回值顺序
- 单行定义的对象结尾不以逗号结尾，多行定义的对象，最后一个成员以逗号结尾，对象尽量静态化，一旦定义，就不得随意添加新的属性，如果添加属性不可避免，要使用 Object.assign 方法，如果对象的属性名是动态的，可以再创建对象时使用属性表达式定义，对象的属性和方法尽量采用简洁表达式
- 使用扩展运算符复制数组，使用 Array.from 将类似数组的对象转为数组
- 立即执行函数写成箭头表达式形式，那些需要使用函数表达式的场合，尽量使用箭头函数表示，因为这样更简洁且绑定了 this，箭头函数取代 Function.prototype.bind,不应该再用 self/\_this/that 绑定 this，简单单但行的不会复用的函数，见建议采用箭头函数，如果函数体较为复杂，行数较多还是应该采用传统写法所有配置项集中放在一个对象中，放在最后一个参数中，布尔值不可以直接作为参数，不要使用 arguments 变量而应该使用...显示表示我们想要获取的参数，使用默认值语法设置函数参数默认参数
- 注意区分 Object 和 Map，只有模拟实体对象时才用到 Object 对象，如果只是需要 key:value 的数据结构，则使用 Map，因为 Map 有便利机制
- 总是 class 取代 prototype 的操作，因为 class 的写法更简洁更易于理解，使用 extends 实现继承
- Module 语法是 javascript 模块的标准写法，要坚持使用这种写法，使用 import 取代 require 写法，使用 export 取代 module.exports,如果模块只有一个输出值，就使用 export default，如果有多个输出值，就不适用 export default，不要同时使用 export default 和 export，如果模块默认输出一个函数，函数名的首字母应该小写，如果默认输出一个对象，对象的首字母应该大写
- 使用 eslint，使用 Airbnb 语法规则
